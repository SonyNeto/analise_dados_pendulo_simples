...
#p1 = scipy.signal.find_peaks(datam1, distance=1500)[0] #Achando os mínomos
...
plt.plot(x, y, c='black', ms=1, label='Ajuste')
plt.plot(df['t'], df['theta'],'b-o', mfc='w', mec='b', ms=1, label='Dados brutos')
plt.xlabel('$t (s)$')
plt.ylabel(r'$x(t)$')
plt.grid()
plt.legend()
