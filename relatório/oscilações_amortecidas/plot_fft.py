###CÓDIGO PARA O MELHOR VALOR DE TRUNCAMENTO PARA A MAIOR FREQUÊNCIA NATURAL DE OSCILAÇÃO###
import sys
...
##Função para o Algoritmo do Melhor Valor bf de Truncamento
def freqop(s, bf):
    df = pd.read_csv(sys.argv[1], sep =',', names=['t', 'x'], header=2)#Leitura dos Dados para DataFrame
    df = df.truncate(before=bf, after=500000)#Truncamento
    df['Média móvel'] = df['x'].rolling(s).mean()#Filtro Móvel
...
#ax.set_title('Transformada de Fourier',fontsize=14)
    ...
    return f_max
    print(bf)
##Achando o Melhor Valor de bf para o Truncamento e Maior Frequência Natural de Oscilação
freqop(414, 483000)
#a = np.array([0])
