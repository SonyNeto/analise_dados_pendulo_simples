###CÓDIGO PARA FILTRO DE MÉDIA MÓVEL, PLOTAGEM DE GRÁFICO E TRANSFORMADA DE FOURIER###
# -*- coding: utf-8 -*-
#Locale settings
import locale, sys
...
def freqop(s):
    ...
    df['Média móvel'] = df['theta'].rolling(s).mean()#Filtro Móvel
    dft = df['t']#DataFrame dos Valores de Tempos
    dfm = pd.merge(dft, df['Média móvel'].dropna(), right_index=True, left_index=True)#DataFrame da Média Móvel com o Tempo
    arq = sys.argv[1].replace('.csv','') + 'Filtrado.csv'#Definindo Nome do Arquivo
    dfm.to_csv(arq, index=False)#Salvando Arquivo
   #print(dfm)
...
# Leitura de dados
    ...
    nPer = 1704 #1491
    nPoints = 180*nPer
    time, signal = lerDados(str(sys.argv[1].replace('.csv','') +'Filtrado.csv'))
    time = time[-nPoints:]
    time = time-time[0]
...
#C = np.amax(signal[15*int(len(signal)/16):])
    x = np.linspace(0,12,306720)
    y = (0.0216938+0.488275)*np.exp(-0.223449*x)*np.cos(2*np.pi*26.472*x+1.84)#Função da Posição pelo Tempo para o Ajuste
    ...
    return ga_fit
freqop(414)
##Achando o Melhor Valor da Janela do Filtro de Média Móvel
#m=512
#while (freqop(m)-freqop(m-1))>=0:
 #      m=m-1
#print(freqop(m))
#a = np.array([999])
